...
describe('Admin Controller', () => {
    ...
    describe('createRole', () => {
        it('should create a role successfully', async () => {
            ...
            Role.findOrCreate.mockResolvedValue([{ id: 1, name: 'Admin' }, true]);
            await createRole(mockRequest, mockReply);
            expect(Role.findOrCreate).toHaveBeenCalledWith({ where: { name: 'Admin' } });
            expect(mockReply.status).toHaveBeenCalledWith(201);
            expect(mockReply.send).toHaveBeenCalledWith({ id: 1, name: 'Admin' });
        });

        it('should return 400 if role name is missing', async () => {
            await createRole(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(400);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Role name is required.' });
        });

        it('should return 409 if role already exists', async () => {
            mockRequest.body.name = 'Admin';
            Role.findOrCreate.mockResolvedValue([{ id: 1, name: 'Admin' }, false]);
            await createRole(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(409);
            expect(mockReply.send).toHaveBeenCalledWith({ message: "Role 'Admin' already exists." });
        });
        ...
        it('should return 500 on error', async () => {
            ...
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error creating role.' });
        });
    });
    ...
    describe('getAllRoles', () => {
        it('should return all roles', async () => {
            ...
            expect(Role.findAll).toHaveBeenCalledWith({ include: [{ model: Permission, through: { attributes: [] } }] });
        ...
        it('should return 500 on error', async () => {
            ...
            await getAllRoles(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error fetching roles.' });
        });
    });
    ...
    describe('getRoleById', () => {
        it('should return a role by ID', async () => {
            ...
            expect(Role.findByPk).toHaveBeenCalledWith(1, { include: [{ model: Permission, through: { attributes: [] } }] });
        ...
        it('should return 404 if role not found', async () => {
            ...
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Role not found.' });
        });
        ...
        it('should return 500 on error', async () => {
            ...
            await getRoleById(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error fetching role.' });
        });
    });
    ...
    describe('deleteRole', () => {
        ...
        it('should delete a role successfully', async () => {
            ...
            expect(Role.findByPk).toHaveBeenCalledWith(1, { transaction: mockTransaction });
            expect(mockRoleInstance.getUsers).toHaveBeenCalledWith({ transaction: mockTransaction });
            expect(mockRoleInstance.setPermissions).toHaveBeenCalledWith([], { transaction: mockTransaction });
            expect(mockRoleInstance.destroy).toHaveBeenCalledWith({ transaction: mockTransaction });
        ...
        it('should return 400 if role is assigned to users', async () => {
            ...
            const mockRoleInstance = {
                id: 1,
                name: 'Tester',
                getUsers: jest.fn().mockResolvedValue([{ id: 1, name: 'User1' }]), // Role assigned to a user
            };
            Role.findByPk.mockResolvedValue(mockRoleInstance);
            ...
            expect(mockTransaction.rollback).toHaveBeenCalled();
            expect(mockReply.status).toHaveBeenCalledWith(400);
            expect(mockReply.send).toHaveBeenCalledWith({ message: "Role 'Tester' cannot be deleted as it is assigned to 1 user(s)." });
        });
        ...
        it('should return 500 on error during findByPk', async () => {
            ...
            expect(mockTransaction.rollback).toHaveBeenCalled();
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error deleting role.' });
        });
        ...
        it('should return 500 on error during setPermissions', async () => {
            ...
            const mockRoleInstance = {
                ...
                name: 'Tester',
                getUsers: jest.fn().mockResolvedValue([]),
                setPermissions: jest.fn().mockRejectedValue(new Error('DB Error')),
            };
            Role.findByPk.mockResolvedValue(mockRoleInstance);
            await deleteRole(mockRequest, mockReply);
            expect(mockTransaction.rollback).toHaveBeenCalled();
    ...
    describe('createPermission', () => {
        it('should create a permission successfully', async () => {
            ...
            Permission.findOrCreate.mockResolvedValue([{ id: 1, name: 'edit_post' }, true]);
            await createPermission(mockRequest, mockReply);
            expect(Permission.findOrCreate).toHaveBeenCalledWith({ where: { name: 'edit_post' }, defaults: { description: 'Can edit posts' } });
            expect(mockReply.status).toHaveBeenCalledWith(201);
            expect(mockReply.send).toHaveBeenCalledWith({ id: 1, name: 'edit_post' });
        });

        it('should return 400 if permission name is missing', async () => {
            await createPermission(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(400);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Permission name is required.' });
        });

        it('should return 409 if permission already exists', async () => {
            mockRequest.body = { name: 'edit_post' };
            Permission.findOrCreate.mockResolvedValue([{ id: 1, name: 'edit_post' }, false]);
            await createPermission(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(409);
            expect(mockReply.send).toHaveBeenCalledWith({ message: "Permission 'edit_post' already exists." });
        });
        ...
        it('should return 500 on error', async () => {
            ...
            await createPermission(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error creating permission.' });
        });
    });
    ...
    describe('getAllPermissions', () => {
        ...
        it('should return 500 on error', async () => {
            ...
            await getAllPermissions(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error fetching permissions.' });
        });
    });
    ...
    describe('assignPermissionToRole', () => {
        it('should assign permission to role successfully', async () => {
            ...
            const mockRole = { id: 1, name: 'Admin', addPermission: jest.fn().mockResolvedValue(undefined) };
            ...
            expect(mockReply.send).toHaveBeenCalledWith({ message: "Permission 'edit_post' assigned to role 'Admin'." });
        });
        ...
        it('should return 404 if permission not found', async () => {
            ...
            await assignPermissionToRole(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(404);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Permission not found.' });
        });
        ...
        it('should return 500 on error', async () => {
            ...
            await assignPermissionToRole(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error assigning permission.' });
        });
    });
    ...
    describe('removePermissionFromRole', () => {
        it('should remove permission from role successfully', async () => {
            mockRequest.params = { roleId: 1, permissionId: 1 };
            const mockRole = { id: 1, name: 'Admin', removePermission: jest.fn().mockResolvedValue(undefined) };
            const mockPermission = { id: 1, name: 'edit_post' };
            Role.findByPk.mockResolvedValue(mockRole);
            Permission.findByPk.mockResolvedValue(mockPermission);
            ...
            expect(mockRole.removePermission).toHaveBeenCalledWith(mockPermission);
            expect(mockReply.status).toHaveBeenCalledWith(200);
            expect(mockReply.send).toHaveBeenCalledWith({ message: "Permission 'edit_post' removed from role 'Admin'." });
        });
        ...
        it('should return 500 on error', async () => {
            mockRequest.params = { roleId: 1, permissionId: 1 };
            Role.findByPk.mockResolvedValue({ id: 1, name: 'Admin', removePermission: jest.fn().mockRejectedValue(new Error('DB Error')) });
            Permission.findByPk.mockResolvedValue({ id: 1, name: 'edit_post' });
            await removePermissionFromRole(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error removing permission.' });
        });
    });
    ...
    describe('assignRoleToUser', () => {
        it('should assign role to user successfully', async () => {
            ...
            const mockUser = { id: 1, addRole: jest.fn().mockResolvedValue(undefined) };
            ...
            expect(mockReply.status).toHaveBeenCalledWith(200);
            expect(mockReply.send).toHaveBeenCalledWith({ message: "Role 'Admin' assigned to user ID 1." });
        });
        ...
        it('should return 500 on error', async () => {
            ...
            await assignRoleToUser(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error assigning role.' });
        });
    });
    ...
    describe('removeRoleFromUser', () => {
        it('should remove role from user successfully', async () => {
            mockRequest.params = { userId: 1, roleId: 1 };
            const mockUser = { id: 1, removeRole: jest.fn().mockResolvedValue(undefined) };
            const mockRole = { id: 1, name: 'Admin' };
            User.findByPk.mockResolvedValue(mockUser);
            Role.findByPk.mockResolvedValue(mockRole);
            ...
            expect(mockReply.status).toHaveBeenCalledWith(200);
            expect(mockReply.send).toHaveBeenCalledWith({ message: "Role 'Admin' removed from user ID 1." });
        });
        ...
        it('should return 500 on error', async () => {
            mockRequest.params = { userId: 1, roleId: 1 };
            User.findByPk.mockResolvedValue({ id: 1, removeRole: jest.fn().mockRejectedValue(new Error('DB Error')) });
            Role.findByPk.mockResolvedValue({ id: 1, name: 'Admin' });
            await removeRoleFromUser(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error removing role.' });
        });
    });
    ...
    describe('getUserRoles', () => {
        it('should return user roles successfully', async () => {
            ...
            expect(User.findByPk).toHaveBeenCalledWith(1, { include: [{ model: Role, through: { attributes: [] } }] });
        ...
        it('should return 500 on error', async () => {
            ...
            await getUserRoles(mockRequest, mockReply);
            expect(mockReply.status).toHaveBeenCalledWith(500);
            expect(mockReply.send).toHaveBeenCalledWith({ message: 'Internal Server Error fetching user roles.' });
        });
    });
});
