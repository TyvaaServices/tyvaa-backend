...
jest.unstable_mockModule('../../src/modules/notification-module/templates.js', () => ({
    __esModule: true,
...
jest.unstable_mockModule('../../src/modules/notification-module/routes/notificationRouter.js', () => ({
    __esModule: true,
    sendFCM: mockSendFCM,
...
describe('Notification Module Server', () => {
    ...
    beforeEach(async () => {
        ...
        notificationModuleServer = (await import('../../src/modules/notification-module/server.js')).default;
    ...
    describe('Message Processing via Broker Subscription', () => {
        ...
        const mockAmqpMessage = { properties: { messageId: 'amqp-test-id-123' } };

        beforeEach(() => {
            // Capture the subscribe callback
            if (mockBrokerInstance.subscribe.mock.calls.length > 0) {
                subscribeCallback = mockBrokerInstance.subscribe.mock.calls[0][1];
            } else {
                throw new Error("Broker subscribe was not called, cannot capture callback.");
            }
        });

        it('should process a valid message, send FCM, and acknowledge', async () => {
            const messagePayload = { token: 'valid-token', eventType: 'USER_WELCOME', data: { name: 'Test User', language: 'en' } };
            mockGetNotificationTemplate.mockReturnValue({ title: 'Welcome!', body: 'Hello Test User' });
            ...
            expect(mockGetNotificationTemplate).toHaveBeenCalledWith('USER_WELCOME', messagePayload.data, 'en');
            expect(mockSendFCM).toHaveBeenCalledWith('valid-token', 'Welcome!', 'Hello Test User', messagePayload.data);
            expect(mockBrokerInstance.acknowledge).toHaveBeenCalledWith(mockAmqpMessage);
        ...
        it('should nack if required fields (token) are missing from payload', async () => {
            const messagePayload = { eventType: 'USER_WELCOME', data: { name: 'Test User' } }; // Missing token

            await subscribeCallback(messagePayload, mockAmqpMessage);

            expect(mockGetNotificationTemplate).not.toHaveBeenCalled();
            expect(mockSendFCM).not.toHaveBeenCalled();
            expect(mockBrokerInstance.nack).toHaveBeenCalledWith(mockAmqpMessage, false, false);
        ...
        it('should nack if template is not found for eventType', async () => {
            const messagePayload = { token: 'valid-token', eventType: 'UNKNOWN_EVENT', data: { language: 'en' } };
            mockGetNotificationTemplate.mockReturnValue(null); // Simulate template not found

            await subscribeCallback(messagePayload, mockAmqpMessage);

            expect(mockGetNotificationTemplate).toHaveBeenCalledWith('UNKNOWN_EVENT', messagePayload.data, 'en');
            expect(mockSendFCM).not.toHaveBeenCalled();
            expect(mockBrokerInstance.nack).toHaveBeenCalledWith(mockAmqpMessage, false, false);
            expect(mockBrokerInstance.acknowledge).not.toHaveBeenCalled();
            expect(fastify.log.error).toHaveBeenCalledWith(
        ...
        it('should nack on top-level error within callback', async () => {
            const messagePayload = { token: 'valid-token', eventType: 'USER_WELCOME', data: { language: 'en' } };
    ...
    it('should log an error if broker connection fails', async () => {
        ...
        mockBrokerInstance.connect.mockRejectedValueOnce(new Error("Broker connect failed"));

        // Re-import and re-register with the failing mock
        notificationModuleServer = (await import('../../src/modules/notification-module/server.js')).default;
        fastify = Fastify(); // New instance
        fastify.log = { info: jest.fn(), error: jest.fn(), warn: jest.fn(), debug: jest.fn() };
        await fastify.register(notificationModuleServer);
        try {
            await fastify.ready();
        } catch (e) {
     ...
     it('should log an error if broker subscription fails', async () => {
        mockBrokerInstance.subscribe.mockRejectedValueOnce(new Error("Subscription failed"));
