...
describe("RBAC Plugin", () => {
    ...
    describe("checkPermission Handler", () => {
        ...
        it("should return 403 if userInstance not found by User.findByPk", async () => {
            ...
            expect(mockReply.send).toHaveBeenCalledWith({
                message: "Forbidden. User not found or invalid.",
            });
            expect(mockRequest.log.warn).toHaveBeenCalledWith(
                "RBAC: User with ID 1 not found in database."
            );
        });
        ...
        it("should return 403 if user does not have permission", async () => {
            ...
            expect(mockUserInstance.hasPermission).toHaveBeenCalledWith(
                "test_permission"
            );
            expect(mockRequest.log.info).toHaveBeenCalledWith(
                "RBAC: User 1 denied access to resource requiring permission 'test_permission'."
            );
        });
        ...
        it("should handle error from User.findByPk", async () => {
            User.findByPk = jest.fn().mockRejectedValue(new Error("DB error"));
            try {
                await checkPermissionHandler(mockRequest, mockReply);
            } catch (e) {}
            expect(mockReply.send).not.toHaveBeenCalledWith(
                expect.objectContaining({
                    message:
                        "Forbidden. Required permission: 'test_permission'.",
                })
            );
        });
        it("should handle error from userInstance.hasPermission", async () => {
            mockUserInstance.hasPermission.mockRejectedValue(
    ...
    describe("checkRole Handler", () => {
        ...
        it("should return 500 if User model not found on fastify.models", async () => {
            ...
            expect(mockRequest.log.error).toHaveBeenCalledWith(
                "RBAC: User model not found for role check."
            );
        });
        ...
        it("should return 403 if userInstance not found by User.findByPk", async () => {
            ...
            expect(mockRequest.log.warn).toHaveBeenCalledWith(
                "RBAC: User with ID 1 not found for role check."
            );
        });
        ...
        it("should return 403 if user does not have role", async () => {
            ...
            expect(mockRequest.log.info).toHaveBeenCalledWith(
                "RBAC: User 1 denied access to resource requiring role 'test_role'."
            );
        });
        ...
        it("should handle error from userInstance.hasRole", async () => {
            mockUserInstance.hasRole.mockRejectedValue(
                new Error("Check failed")
            );
            try {
                await checkRoleHandler(mockRequest, mockReply);
