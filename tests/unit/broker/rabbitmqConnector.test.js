...
describe('RabbitMQ Connector', () => {
    ...
    describe('getChannel', () => {
        ...
        it('should retry connection on failure up to MAX_RETRIES', async () => {
            ...
            mockAmqp.connect.mockRejectedValueOnce(new Error('Connection failed 1'))
            ...
            const originalRetryDelay = 5000; // Value from module, adjust if changed
            const tempModule = await import('../../src/broker/rabbitmqConnector.js'); // Re-import not ideal, better to make RETRY_DELAY_MS configurable or small for test env
            ...
            mockAmqp.connect.mockImplementation(originalConnect || jest.fn().mockResolvedValue(mockConnection));
        }, 15000); // Increase timeout for this test due to retries

        it('should throw error after MAX_RETRIES failures', async () => {
            mockAmqp.connect.mockRejectedValue(new Error('Persistent connection failure'));

            // This test will also be slow.
            await expect(rabbitmqConnector.getChannel()).rejects.toThrow('Persistent connection failure');
