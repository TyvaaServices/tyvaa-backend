...
describe('Broker (RabbitMQ)', () => {
    ...
    describe('Connection', () => {
        ...
        it('should emit error if connect fails', async () => {
            mockGetChannel.mockRejectedValueOnce(new Error('Connection failed'));
    ...
    describe('assertQueue', () => {
        it('should call channel.assertQueue', async () => {
            ...
            expect(mockBrokerChannel.assertQueue).toHaveBeenCalledWith('test-queue', { durable: false });
        });
        ...
        it('should attempt to connect if channel not available', async () => {
            ...
            expect(mockBrokerChannel.assertQueue).toHaveBeenCalledWith('test-queue', { durable: true });
        });
    });
    ...
    describe('Publishing', () => {
        ...
        it('sendToQueue should assert queue and send message', async () => {
            ...
            expect(mockBrokerChannel.assertQueue).toHaveBeenCalledWith(queueName, { durable: true });
            expect(mockBrokerChannel.sendToQueue).toHaveBeenCalledWith(
                queueName,
                Buffer.from(JSON.stringify(message)),
                { persistent: true }
            );
            expect(broker).toHaveEmitted('message-published', { queueName, msg: message });
        });
        ...
        it('publish should call channel.publish', async () => {
            ...
            expect(broker).toHaveEmitted('message-published', { exchangeName: exchange, routingKey, msg: message });
        });
        ...
        it('should throw error if trying to publish when not connected', async () => {
            ...
            await expect(broker.sendToQueue('q', {}))
                .rejects.toThrow("Broker not connected. Cannot send to queue.");
        });
    });
    ...
    describe('Subscription', () => {
        ...
        it('subscribe should assert queue and consume messages', async () => {
            ...
            expect(mockBrokerChannel.assertQueue).toHaveBeenCalledWith(queueName, { durable: true });
            expect(mockBrokerChannel.consume).toHaveBeenCalledWith(queueName, expect.any(Function), { noAck: false });
            expect(consumerTag).toBe('mock-consumer-tag');
            expect(broker).toHaveEmitted('subscribed', { queueName, consumerTag });
        ...
        it('subscribe callback should nack unparseable message', async () => {
            ...
            expect(mockBrokerChannel.nack).toHaveBeenCalledWith(mockInvalidMsg, false, false);
        });
        ...
        it('unsubscribe should cancel consumer', async () => {
            ...
            expect(broker).toHaveEmitted('unsubscribed', { consumerTag, queueName: 'some-queue' });
        });
    });
    ...
    describe('Acknowledgement', () => {
        ...
        it('acknowledge should call channel.ack', () => {
            ...
            expect(broker).toHaveEmitted('message-acknowledged', { messageId: 'test-msg-id' });
        });

        it('nack should call channel.nack', () => {
            broker.nack(mockAmqpMessage, false, true);
            expect(mockBrokerChannel.nack).toHaveBeenCalledWith(mockAmqpMessage, false, true);
            expect(broker).toHaveEmitted('message-nacked', { messageId: 'test-msg-id', requeue: true });
        });
    });
    ...
    describe('Purge Queue', () => {
        ...
        it('purgeQueue should call channel.purgeQueue', async () => {
            ...
            mockBrokerChannel.purgeQueue.mockResolvedValueOnce({ messageCount: 10 });
            const result = await broker.purgeQueue(queueName);
            expect(mockBrokerChannel.purgeQueue).toHaveBeenCalledWith(queueName);
            expect(result.messageCount).toBe(10);
            expect(broker).toHaveEmitted('queue-purged', { queueName, messageCount: 10 });
        });
    });
...
expect.extend({
    toHaveEmitted(received, eventName, eventArgs) {
        ...
        const relevantEmits = mockLogger.info.mock.calls.filter(call =>
            call[0].includes(`Emitted event: ${eventName}`) || // If you had specific logging for emits
            (brokerInstance.emit.mock && brokerInstance.emit.mock.calls.find(emitCall => emitCall[0] === eventName))
        );
        ...
        const brokerEmitMock = brokerInstance.emit = jest.spyOn(brokerInstance, 'emit');
        ...
        if (found) {
            return { pass: true, message: () => `event ${eventName} emitted as expected` };
        }
        return { pass: false, message: () => `expected event ${eventName} to be emitted with ${eventArgs ? JSON.stringify(eventArgs) : 'any arguments'}. Actual emits: ${JSON.stringify(brokerEmitMock.mock.calls)}` };
    },
});
