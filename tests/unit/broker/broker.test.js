...
describe("Broker (RabbitMQ)", () => {
    beforeEach(async () => {
        ...
        mockBrokerChannel.consume.mockResolvedValue({
    ...
    afterEach(async () => {
        if (broker && typeof broker.destroy === "function") {
            await broker
                .destroy()
                .catch((err) =>
                    mockLogger.error("Error during afterEach destroy:", err)
                );
        }
    ...
    describe("Constructor and Singleton", () => {
        ...
        it("getInstance should return the same instance (already created in beforeEach)", () => {
            ...
            const instance2 = BrokerModule.default.getInstance({
                new: "config",
            });
        ...
        it("getInstance (called first time in a fresh module context) should create a new instance with config", async () => {
            ...
            expect(instance).toBeInstanceOf(
                FreshBrokerModule.default.BrokerClass
            );
    ...
    describe("Connection", () => {
        it("should connect, get channel and connection on connect()", async () => {
            ...
            expect(mockBrokerConnection.on).toHaveBeenCalledWith(
                "error",
                expect.any(Function)
            );
            expect(mockBrokerConnection.on).toHaveBeenCalledWith(
        ...
        it("should emit 'error' and nullify channel when connection emits 'error'", async () => {
            ...
            const errorHandler = mockBrokerConnection.on.mock.calls.find(
                (call) => call[0] === "error"
            )[1];
            ...
            expect(mockLogger.error).toHaveBeenCalledWith(
                "Broker: RabbitMQ connection error:",
                "RabbitMQ connection died"
            );
        ...
        it("should emit 'close' and nullify channel when connection emits 'close'", async () => {
            ...
            const closeHandler = mockBrokerConnection.on.mock.calls.find(
                (call) => call[0] === "close"
            )[1];
            closeHandler();
            ...
            expect(mockLogger.warn).toHaveBeenCalledWith(
                "Broker: RabbitMQ connection closed."
            );
        });
        ...
        it("should emit error if connect fails (getConnection rejects)", async () => {
            mockGetConnection.mockRejectedValueOnce(
                new Error("Connection failed to get actual connection")
            );

            await expect(broker.connect()).rejects.toThrow(
                "Connection failed to get actual connection"
            );
            expect(broker.emit).toHaveBeenCalledWith(
                "error",
                expect.any(Error)
    ...
    describe("assertQueue", () => {
        ...
        it("should emit error and rethrow if assertQueue fails", async () => {
            ...
            await expect(broker.assertQueue("fail-queue")).rejects.toThrow(
                "Failed to assert"
            );
    ...
    describe("Publishing", () => {
        ...
        it("publish should throw if connect resolves but channel is still null (highly defensive)", async () => {
            ...
            expect(freshBroker.emit).toHaveBeenCalledWith(
                "error",
                expect.any(Error)
            );
