...
async function seed() {
  ...
  const passengerProfiles = await Promise.all(usersWithRoles.map(({ user, roles }) =>
    roles.includes('PASSAGER') ? PassengerProfile.create({ userId: user.id }) : null
  ));
  const driverProfiles = await Promise.all(usersWithRoles.map(({ user, roles }) =>
    roles.includes('CHAUFFEUR') ? DriverProfile.create({ userId: user.id, statusProfile: 'Active' }) : null
  ));
  ...
  await Promise.all(validPassengerProfiles.map(profile =>
